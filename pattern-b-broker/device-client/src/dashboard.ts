...
async function getBrokerAuthHeaders(): Promise<Record<string, string>> {
  if (AUTH_METHOD === "entra-id") {
    ...
    return { authorization: `Bearer ${tokenResponse.token}`, "x-auth-method": "entra-id" };
  }
  return { authorization: `Bearer ${BROKER_API_KEY}`, "x-auth-method": "api-key" };
}

async function requestTokenFromBroker(): Promise<TokenResponse> {
  const headers = await getBrokerAuthHeaders();
...
app.post("/api/request-token", async (_req, res) => {
  try {
    const result = await requestTokenFromBroker();
...
app.post("/api/read-contents", async (_req, res) => {
  if (!currentToken) return res.status(400).json({ success: false, error: "No token — request one first" });
  try {
...
app.post("/api/create-issue", async (_req, res) => {
  ...
  try {
    const issue = await createIssue(
      ...
      "This issue was created from the Pattern B dashboard demo (broker-mediated)."
    );
...
app.post("/api/push-commit", async (_req, res) => {
  ...
  try {
    const result = await pushCommit(
      ...
      `Device diagnostic report\nTimestamp: ${new Date().toISOString()}\nSource: Pattern B Dashboard (broker)\nAuth: ${AUTH_METHOD}\n`,
      "chore: device diagnostic upload (Pattern B dashboard)"
    );
...
<title>Pattern B — Token Broker Demo</title>
<style>
  :root {
    --bg: #0d1117; --surface: #161b22; --border: #30363d;
    --text: #e6edf3; --muted: #7d8590; --accent: #a371f7;
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .badge-purple { background: rgba(163,113,247,.15); color: var(--accent); }
  ...
  .broker-info { display: flex; gap: 1rem; flex-wrap: wrap; margin-bottom: .75rem; font-size: .8rem; }
  .broker-info span { background: var(--bg); padding: 4px 10px; border-radius: 4px; }
  ...
  .log-broker { color: var(--accent); }
</style>
</head>
<body>

<h1>🔐 Pattern B — Token Broker Demo</h1>
<p class="subtitle">Device authenticates to a trusted broker, which mints short-lived GitHub tokens. No GitHub secrets on device.</p>

<div class="flow" id="flow">
  <div class="flow-step" id="step-device">📱 Device</div>
  <div class="flow-arrow">→</div>
  <div class="flow-step" id="step-auth">Authenticate</div>
  <div class="flow-arrow">→</div>
  <div class="flow-step" id="step-broker">🔐 Token Broker</div>
  <div class="flow-arrow">→</div>
  <div class="flow-step" id="step-mint">Mint Token</div>
  <div class="flow-arrow">→</div>
...
<div class="card">
  <h2>🔑 Token from Broker</h2>
  <div class="broker-info">
    <span>Broker: <strong id="broker-url"></strong></span>
    <span>Auth: <strong id="auth-method"></strong></span>
  </div>
  <div class="btn-row">
    <button id="btn-request" onclick="requestToken()">Request Token from Broker</button>
...
function log(msg, type = '') {
  ...
  const cls = type === 'ok' ? 'log-ok' : type === 'err' ? 'log-err' : type === 'broker' ? 'log-broker' : '';
...
async function requestToken() {
  ...
  setFlowActive('step-device', 'step-auth');
  log('Authenticating to broker (' + document.getElementById('auth-method').textContent + ')...', 'broker');

  try {
    setFlowActive('step-auth', 'step-broker');
    log('Sending token request to broker...', 'broker');
    setFlowActive('step-broker', 'step-mint');
    const res = await fetch('/api/request-token', { method: 'POST' });
    const data = await res.json();
    if (!data.success) throw new Error(data.error);

    setFlowActive('step-mint');
    ...
    log('Broker minted token: ' + data.masked_token + ' (expires ' + data.expires_at + ')', 'ok');
    showResult('mint-result', data, true);
  } catch (err) {
    log('Broker request failed: ' + err.message, 'err');
    showResult('mint-result', { error: err.message }, false);
  }
  btn.disabled = false;
  btn.textContent = 'Request Token from Broker';
}
...
async function runOp(op, btn) {
  ...
  setFlowActive('step-mint', 'step-github', 'step-ops');
  log('Running: ' + op + '...');
...
fetch('/api/status').then(r => r.json()).then(d => {
  ...
  document.getElementById('broker-url').textContent = d.brokerUrl;
  document.getElementById('auth-method').textContent = d.authMethod;
  if (d.hasToken) {
    document.getElementById('token-info').style.display = 'flex';
...
app.listen(PORT, () => {
  console.log(`\n🔐 Pattern B Device Dashboard: http://localhost:${PORT}`);
